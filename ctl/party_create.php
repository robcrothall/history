<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST")
    {
		...
		$row = $rows[0];
		if($row["rowCount"] > 0) {$message .= "This party_name already exists in our records."; $error = true;}
		// If no errors have been detected, try to insert the row after noting some warnings
		if($error == false) 
		{
			$party_name = trim($party_name);
			$notes = trim($notes);
			$rowCount = query("insert into party (party_name, party_leader, notes, user_id, changed) values (?,?,?,?,CURRENT_TIMESTAMP())",$party_name, $party_leader,$notes,$_SESSION["id"]);
			if($rowCount == false) 
			{
