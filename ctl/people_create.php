<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST")
    {
      ...
      else {$surname=trim(substr(htmlspecialchars(strip_tags($_POST['surname']), ENT_COMPAT),0,50));}
      ...
      $rows = query("insert into people (surname, first_name, other_name, title, birth_year, occupation_id, party_id, ref_no, notes, user_id, changed) values (?,?,?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP())",
      					$surname, $first_name, $other_name, $title, $birth_year, $occupation_id, $party_id, $ref_no, $notes, $user_id);
		if($rows !== false) 
			{
